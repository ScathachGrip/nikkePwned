...
function buildF8ListenerScript(parentPid: number): string {
  return [
    ...
    "$hasModifier=(([KeyboardState]::GetAsyncKeyState($vkCtrl) -band 0x8000) -ne 0) -or (([KeyboardState]::GetAsyncKeyState($vkAlt) -band 0x8000) -ne 0) -or (([KeyboardState]::GetAsyncKeyState($vkShift) -band 0x8000) -ne 0) -or (([KeyboardState]::GetAsyncKeyState($vkLWin) -band 0x8000) -ne 0) -or (([KeyboardState]::GetAsyncKeyState($vkRWin) -band 0x8000) -ne 0)",
    "if($isDown -and -not $wasDown -and -not $hasModifier){",
    "try{",
